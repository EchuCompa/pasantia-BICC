...
Proposition~\ref{prop:bound} indicates that the number of equivalence classes will be small when the tree is shallow. From the point of view of the causal graph, this corresponds to the case in which there aren't long causal dependencies between the features. In particular, there will be many features that are mutually independent. In our experimentation we used balanced trees, multiple path graphs, Naive Bayes and other classes of graphs which don't have a very long causal dependencies. 
...
\echu{Ofrezco dos opciones, una más por arriba y otra que entra más en detalle. Choose your posion. PD: No puse nada de la complejidad pero no se cómo poner algo sin que sea humo, porque no estoy explicando nada.}
...
\begin{proof}
	...
	OPCION 2:
	We first focus on the unrelated part: for each subtree rooted at a node in $UR$, we recursively compute its equivalence classes. This is done by combining the classes of its children and deciding whether each node appears to the left or to the right of $x_i$, which fully determines its contribution. As a result, each unrelated subtree can be processed independently, yielding a set of partial equivalence classes together with their associated counts.
	
	We then combine these classes across all unrelated subtrees and integrate them with the ancestors and descendants. Descendants are always placed to the right of $x_i$ and can therefore be treated as a block, whose contribution is given by the number of topological orders of the corresponding subtree. In contrast, ancestors impose precedence constraints, so we must count the valid interleavings between them and the nodes placed to the left. Altogether, this construction yields all equivalence classes and their sizes without explicitly enumerating topological orders.
\end{proof}
...
\echu{Acá también dejo dos opciones, una más en detalle y otra más por arriba. }
...
The algorithm computes the number of topological orderings of a polytree by performing a DFS over its underlying graph and recursively combining the results of each node’s unvisited neighbors. At each step, it considers all valid relative orders of the node and its neighbors, combines their contributions, and merges equal positions. This yields an exact counting algorithm for polytrees without explicitly enumerating all topological orders.
...
\begin{figure}[ht]
	...
	\end{tikzpicture}
	\caption{Illustration of the first step of the sampling algorithm. The candidate source nodes that may appear first in the ordering are shown in red, while the rest of the graph is shown in blue. Each source node is annotated with the number of topological orderings in which it appears first.}
	\label{fig:topoSortSamplingExample}
\end{figure}

Alternatively, we consider known algorithms from the literature able to approximate uniform sampling of topological orderings from arbitrary DAGs. We implement an algorithm based on random walks \parencite{bubley1999faster} with a polynomial (although of high degree) complexity. We note that there are more modern and asymptotically better algorithms \parencite{huber2006fast}, but we believe that the hidden constants in them make them inefficient for the input sizes we consider.
