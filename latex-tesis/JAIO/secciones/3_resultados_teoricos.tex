...
\begin{proof}
	...
	we put $y$ and have complete freedom to decide which features to put from the sub-trees of $y$. The bound from the statement of the proposition can be derived using Eq.~\eqref{eq:fun_for_equi_classes} (FIGURA DE COMO MERGEAR CLASES DE EQUIVALENCIAS DE UNRELATED). 
\end{proof}

Proposition~\ref{prop:bound} indicates that the number of equivalence classes will be small when the tree is shallow. From the point of the view of the causal graph, this corresponds to the case in which there aren't long causal dependencies between the features. In particular, there will be many features that are mutually independent. (NOMBRAR EJEMPLOS DE ARBOLES PUNTUALES QUE DESPUES VEMOS EN LA EXPERIMENTACION)  
