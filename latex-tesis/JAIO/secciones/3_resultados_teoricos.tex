...
by \textit{rooted directed trees} (EJEMPLO? NOMBRAR ESTO ANTES). In this case, note that we can split the nodes in three groups with respect to $x$: those that are descendants of $x$, those that are ancestors of $x$, and the \textit{unrelated} ones. Observe that both descendant/ancestors are never/always in the set $\pi_{<x}$, for any $x$. Thus, the structure of the 
equivalence classes depends only on the unrelated nodes. (FIGURA)  

We have a general bound that allows to bound the size of $\equivalenceClasses$:
...
\begin{proof}
	...
	\begin{align}\label{eq:equivalence_classes_subtree}
		f(y) = \left| \{S \subseteq C_y : \exists \pi \in perm(C) \text{ such that } \pi_{<x} = S\} \right|	
	\end{align}
	%
	Note that
	%
