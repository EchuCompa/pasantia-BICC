...
\end{align*}
%
This rewritting is obtained by observing that many permutations give raise to the same set $\pi_{<x}$: more precisely, if $|\pi_{<x}| = s$ there are $\frac{s!(n-s-1)!}{n!}$ permutations $\pi'$ satisfying $\pi'_{<x} = \pi_{<x}$, where $n = |X|$. Computing the shapley values using this formula reduces the number of times that $\nu_{M,e}$ has to be computed, although the number of calls remains
...
\end{align}
%
Therefore, instead of invoking $\nu_{M,e}$ $|topos(C)|$ times it is enough to compute it $|\equivalenceClasses|$ times. This last set can have size at most $2^{|X|}$, but the hope is that in pratical scenarios it is smaller. From now on, we will study this set in the context of causal graphs given 
by \textit{rooted directed trees} (EJEMPLO? NOMBRAR ESTO ANTES). In this case, note that we can split the nodes in three groups with respect to $x$: those that are descendants of $x$, those that are ancestors of $x$, and the \textit{unrelated} ones. Observe that descendant/ancestors are never/always in the set $\pi_{<x}$, for any $x$. Thus, the structure of the 
equivalence classes depends only on the unrelated nodes. (FIGURA)  
...
\begin{proof}
	For every unrelated feature $y$ (in the sense that $y$ is neither a descendendant or ancestor of $x$) let $C_y \subseteq C$ be the directed tree rooted at $y$ induced by those nodes reachable by $y$. Let $f(y)$ denote the number of ways that the features in $C_y$ can be arranged in the permutations, i.e.
	%
	...
	we put $y$ and have complete freedom to decide which features to put from the subtrees of $y$. The bound from the statement of the proposition can be derived using Eq.~\eqref{eq:fun_for_equi_classes} (FIGURA DE COMO MERGEAR CLASES DE EQUIVALENCIAS DE UNRELATED). 
\end{proof}

Proposition~\ref{prop:bound} indicates that the number of equivalence classes will be small when the tree is shallow. From the point of the view of the causal graph, this corresponds to the case in which there aren't long causal dependences between the features. In particular, there will be many features that are mutually independent. (NOMBRAR EJEMPLOS DE ARBOLES PUNTUALES QUE DESPUES VEMOS EN LA EXPERIMENTACION)  

For the case of directed trees we further develop an algorthm able to compute the set $\equivalenceClasses$ as well as the number of permutations inside each class (which is required to compute Eq.~\eqref{eq:equivalence_classes_into_shap}). The complexity of the algorithm
is polynomial on $|\equivalenceClasses|$.

\begin{proposition}
	There is an algorithm able to compute $|\equivalenceClass|$ given the causal graph $C$ on time polynomial on $|C|$ and $|\equivalenceClasses|$. Moreover, the algorithm returns the size of each class $[\pi] \in \equivalenceClasses$.
\end{proposition}

\begin{proof}
	The algorithm is recursive on its nature, and based on the ideas from the proof of Proposition~\ref{prop:bound}. (EXPLICAR)
\end{proof}
...
(because the bound from Proposition~\ref{prop:bound} implies that the algorithm take exponential time on the causal graph) is must be done only once. Moreover, it is independent of the model $M$, and thus it can be reused even
...
done. This is the case for the product distribution, but also for some bayesian networks such as a Naive Bayes distributiton. (AGREGAR ALGO. CREO QUE ESTE PROBLEMA TENIA QUE VER CON INFERENCIA BAYESIANA, NO? PONERLO)
...
we pick left-most node of the ordering. In particular, in the beginning we must pick one of the sources of the causal graph (i.e. one of the nodes with in-degree 0). We should weight them according to the number of topological orderings that start with each respective node, and thus we effectively reduce the problem
...
with a polynomial (although of high degree) complexity. We note that there are more modern and asimptotically better algorithms \parencite{huber2006fast}, but we believe that the hidden constants in them make them inefficient for the input sizes we consider. (PODEMOS PONER EL PSEUDO DEL SAMPLEO QUE IMPLEMENTAMOS, PERO PINTA QUE NO HAY ESPACIO)
